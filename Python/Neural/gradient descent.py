...
# Calculating direction and magnitude from error
...
for iteration in range(20):
    ...
    direction_and_amount = (pred - goal_pred) * input  # This
    weight = weight - direction_and_amount

    print("Error: " + str(error) + " Prediction: " + str(pred))

# Перед вами более эффективный метод обучения, известный как градиентный
# спуск. Этот метод позволяет в одной строке кода (выделена #) вычислить сразу и направление, и величину изменения веса weight для уменьшения
# ошибки error.
...
# Чистая ошибка — это разность (pred - goal_pred), определяющая направление и величину промаха. Если это положительное число, значит, прогноз
# слишком велик, и наоборот. Если это большое число, значит, вы сильно промахнулись, и так далее.
...
# трех основных крайних случаев, когда чистой ошибки недостаточно для выбора хорошей величины изменения веса.

# ЧТО ТАКОЕ ОСТАНОВКА

# Остановка — это первый (и самый простой) эффект, обусловленный умножением чистой ошибки на input. Представьте, что вы подключили проигрыватель компакт-дисков к своей стереосистеме. Если теперь включить полную
# громкость, но не включить проигрыватель, изменение громкости ничего не
# даст. Остановка — это похожий эффект в нейронных сетях. Если input получит значение 0, тогда direction_and_amount также получит значение 0.
...
# input имеет положительное значение) смещение веса вверх влечет смещение прогноза тоже вверх. Но если input получит отрицательное значение, вес начнет изменяться в другом направлении! При отрицательном
...
# и вес нужно изменить на большую величину. Это в большей степени побочный эффект, потом что часто выходит из-под контроля. Позднее мы познакомимся с альфа-коэффициентом, который будем использовать, когда такое
# случится.
